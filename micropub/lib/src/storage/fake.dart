...
final fakePackages = <MicropubPackageDetails>[
  _createPackage('example', 1),
  _createPackage('server', 2),
  _createPackage('api_client', 3),
  _createPackage('model', 4),
  _createPackage('design_system', 5),
  for (var i = 1; i < 1000; i++) ...[
    _createPackage('example_$i', 1 + i),
    _createPackage('server_$i', 2 + i),
    _createPackage('api_client_$i', 3 + i),
    _createPackage('model_$i', 4 + i),
    _createPackage('design_system_$i', 5 + i),
  ]
];

MicropubPackageDetails _createPackage(String name, int lastUpdate) {
  return MicropubPackageDetails(
    package: MicropubPackage(
      name: name,
      createdAt: DateTime.now().add(Duration(days: -lastUpdate - 10)),
      updatedAt: DateTime.now().add(Duration(days: -lastUpdate)),
      uploaders: ['john.doe@email.com'],
      download: 128,
      versions: [
        MicropubVersion(
          version: '1.2.1',
...
class MicropubFakeStorage extends MicropubStorage {
  const MicropubFakeStorage({
  ...
  Future<String?> loadReadme(String name) {
    return _delayed(() async {
      return fakePackages
          .firstWhereOrNull((x) => x.package.name == name)
          ?.readme;
    });
  }
  ...
  Future<MicropubQueryResult> queryPackages(
      ...
      String? dependency}) {
    return _delayed(() async {
      return MicropubHiveStorage.filterPackages(
        fakePackages.map((x) => x.package).toList(),
        size: size,
        page: page,
        sort: sort,
        keyword: keyword,
  ...
  Future<T> _delayed<T>(Future<T> Function() execute) async {
    await Future.delayed(simulatedDelay);
    return await execute();
  }
}

const _freezedReadme = r'''
